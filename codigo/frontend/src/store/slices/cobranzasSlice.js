...
export const anularCobranzaThunk = createAsyncThunk(
  'cobranzas/anular',
...
const cobranzasSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(anularCobranzaThunk.fulfilled, (state, action) => {state.loading = false;const cobranzaActualizada = action.payload; const index = state.lista.findIndex(c => c.id === cobranzaActualizada.id);
      if (index !== -1) {state.lista[index] = cobranzaActualizada;}})
